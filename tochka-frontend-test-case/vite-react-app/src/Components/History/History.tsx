...
const History = () => {
  ...
  const type_filter_button_classes = classNames(
    styles.filter_button,
    showFilter === 'type' ? styles.active : ''
  );
  const date_filter_button_classes = classNames(
    styles.filter_button,
    showFilter === 'date' ? styles.active : ''
  );

  //Класы для .filters
  const filters_classes = classNames(
    styles.filters,
    showFilter === false ? '' : styles.active
  );

  // Действия для кнопок, открывающих фильтры
  const handlerTypeFilter = () =>
    showFilter === 'type' ? setShowFilter(false) : setShowFilter('type');
  const handlerDateFilter = () =>
    showFilter === 'date' ? setShowFilter(false) : setShowFilter('date');
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <span>История</span>
        {data.length === 0 && (
          <img src={Pen} alt="" className={styles.not_mobile} />
        )}
        {data.length === 0 && (
          <img src={Plus} alt="" className={styles.mobile} />
        )}
      </div>
        ...
        <div className={filters_classes}>
          <div className={styles.typeFilter}>
            <button
              ...
              className={type_filter_button_classes}
            >
          ...
          <div className={styles.dateFilter}>
            <button
              ...
              className={date_filter_button_classes}
            >
              <span>Период</span>
