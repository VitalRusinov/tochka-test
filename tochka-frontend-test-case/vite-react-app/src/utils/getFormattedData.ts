...
export const getFormattedData = (newData: IEvent[]): IEvent[] => {
  ...
  const firstDate = createDateEl(currentDate, getRelativeDay(today, currentDate));
  ...
  newData.forEach((item) => {
    ...
    if (!isThisIsOneDay(currentDate, formDate)) {
      const dateEl = createDateEl(formDate, getRelativeDay(today, formDate), getYear(currentDate, formDate));
