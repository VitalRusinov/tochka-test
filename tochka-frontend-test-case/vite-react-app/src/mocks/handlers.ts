...
const dataset: Array<Omit<IEvent, 'date'>> = [
  ...
  },
  {
    title: 'Промт, ООО',
    description:
      'Оплата услуг контрагенту по договору подряда № 206 от 12.03.2019, в т. ч. НДС 18%',
    icon: 'Alfabank.svg',
    type: typeEvents.Balance,
    balance: '+ 50 000 ₽',
    optionalDescription: '№6884, 13:40',
  ...
  },
  {
    ...
    description:
      'По решению о взыскании № 4514 от 08.06.2022 на основании ст.46 НК РФ от 31.07.1998г. № 146‑ФЗ без НДС',
...
function eventsResolver({ request, params }) {
  if (countRequest < finalCountRequest) {
    const result = [];
    for (let i = 0; i < 20; ) {
      const countEventsPerDay = Math.round(Math.random() * 5);
      for (let j = 0; j <= countEventsPerDay; j++) {
        const index = Math.round(Math.random() * (dataset.length - 1));
        const event: IEvent = {
          ...dataset[index],
